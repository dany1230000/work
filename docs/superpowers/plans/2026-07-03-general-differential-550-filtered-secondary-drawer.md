...
- [ ] Add a quality test for 550 conditions, 628 sources, zero blockers, zero warnings, and completed `expand_condition_catalog_to_550`.
- [ ] Add a searchability test for the 25 new condition slugs after de-dupe.
- [ ] Add a UI test that secondary candidate cards render inside a filterable drawer after primary candidates.
- [ ] Update exact reviewed catalog count assertions from 525/603 to 550/628.
...
- [ ] Select 25 de-duplicated high-value conditions from vaccine-preventable, zoonotic, fungal, and opportunistic infection gaps.
- [ ] Use official or institutionally authoritative source rows, preferring CDC/WHO/NIH/Merck Professional where available.
- [ ] Add the seventeenth 25-condition batch and extend `CONDITIONS`.
- [ ] Set `EXPANSION_TARGET_CONDITIONS = 550`.
...
- [ ] Add presentation metadata for urgency and system filter counts without changing ranking.
- [ ] Render secondary candidates behind urgency/system filter controls.
- [ ] Add minimal JavaScript to filter rows with stable `data-secondary-filter-*` markers and an empty state.
