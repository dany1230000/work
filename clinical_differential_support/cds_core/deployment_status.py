...
def build_deployment_status_report(
    ...
    return {
        ...
        "external_blockers": [
            check for check in checks if check["status"] in {"action_required", "locked"}
        ],
        "steps": steps,
...
def format_deployment_status_report(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- No clinical production approval and no public deployment claim.",
...
def _build_deployment_checks(
    ...
    return [
        ...
        {
            "check_id": "render_cli",
            "status": "passed" if render_cli_ok else "action_required",
            "title_zh": "Render CLI",
            "title_en": "Render CLI",
            "value": _first_line(render_cli.stdout) if render_cli_ok else "missing",
        ...
        },
        {
            ...
            "status": "passed" if render_auth_ok else ("action_required" if render_cli_ok else "locked"),
            "title_zh": "Render auth",
            "title_en": "Render auth",
            "value": "authenticated" if render_auth_ok else "not_authenticated",
